...
# I-LLM per CONST-035 + CONST-050(B). Submodule cascade per
# CONST-051(A). Targets $ILLM_HEALTH_URL.
...
echo "=== I-LLM DDoS Health-Flood Challenge ==="
echo "  url=$HEALTH_URL total=$TOTAL_REQS conc=$CONCURRENCY pass≥${MIN_PASS_PCT}%"

if [[ -z "$HEALTH_URL" ]]; then
    echo "[1/5] SKIP: ILLM_HEALTH_URL unset — SKIP-OK: #env-no-target"
    echo "=== I-LLM DDoS Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== I-LLM DDoS Challenge: PASSED ==="
