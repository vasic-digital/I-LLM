...
echo "=== I-LLM UI Terminal-Interaction Challenge ==="
echo "  bin=$BIN_PATH timeout=${TIMEOUT_SEC}s"

if [[ -z "$BIN_PATH" ]] || [[ ! -x "$BIN_PATH" ]]; then
    echo "[1/4] SKIP: ILLM_BIN not set — SKIP-OK: #env-binary-missing"
    echo "=== I-LLM UI Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== I-LLM UI Challenge: PASSED ==="
